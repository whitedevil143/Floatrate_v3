...
class FloatingOverlayService : Service() {
    ...
    private val repositoryListener = object : BinanceMarketRepository.Listener {
        override fun onTicker(ticker: Ticker) {
            ...
            overlayPanel?.renderForecast(FiveMinuteForecast.predict(forecastCandles, ticker.lastPrice))
        }
        ...
        override fun onForecastCandles(candles: List<Candle>) {
            ...
            overlayPanel?.renderForecast(FiveMinuteForecast.predict(candles, latestTicker?.lastPrice))
        }
    ...
    private fun resizeOverlay(isCollapsed: Boolean) {
        ...
        windowParams.height = if (collapsed) dp(112) else dp(274)
        overlayPanel?.let {
            try {
...
private class OverlayPanel(
    ...
    init {
        ...
        forecastText = TextView(context).apply {
            text = "5M ESTIMATE · waiting for one-minute candles"
            labelStyle(9f, muted, true)
            setPadding(context.dp(8), context.dp(6), context.dp(8), context.dp(6))
            background = roundedBackground(surfaceElevated, context.dp(9f), outline)
        }
        content.addView(forecastText, overlayMargin(context, top = 7))
    ...
    fun renderForecast(result: ForecastResult?) {
        if (result == null) {
            forecastText.text = "5M ESTIMATE · waiting for one-minute candles"
            forecastText.labelStyle(9f, muted, true)
            forecastText.background = roundedBackground(surfaceElevated, this.context.dp(9f), outline)
            return
        }
        val color = when (result.direction) {
            ForecastDirection.UP -> positive
            ForecastDirection.DOWN -> negative
            ForecastDirection.MIXED -> muted
        }
        val background = when (result.direction) {
            ForecastDirection.UP -> positiveSoft
            ForecastDirection.DOWN -> negativeSoft
            ForecastDirection.MIXED -> surfaceElevated
        }
        forecastText.text = String.format(
            Locale.US,
            "5M EST.  %s  ·  %+.3f%%  ·  %s",
        ...
        forecastText.labelStyle(9f, color, true)
        forecastText.background = roundedBackground(background, this.context.dp(9f), if (result.direction == ForecastDirection.MIXED) outline else null)
    }
