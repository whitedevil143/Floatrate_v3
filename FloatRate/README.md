...
- A **5M ESTIMATE** tab beside the live rate. It calculates an on-device short-horizon estimate from the latest one-minute candles using log momentum, EMA spread, regression slope, and volatility damping. It reports an UP/DOWN/MIXED bias and signal strength, never a guaranteed price.
...
- The 5-minute estimate is intentionally an approximate technical heuristic, not an exact prediction or financial advice. It is recalculated from 60 one-minute candles and is capped to avoid displaying an exaggerated move.
